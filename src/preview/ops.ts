...
type PadOutStitcherCanvas = HTMLCanvasElement & { __imageopsPadOutMask?: HTMLCanvasElement };
...
function markPadOutStitcherCanvas(canvas: HTMLCanvasElement, mask: HTMLCanvasElement): PadOutStitcherCanvas {
  const stitcherCanvas = canvas as PadOutStitcherCanvas;
  stitcherCanvas.__imageopsPadOutMask = mask;
  return stitcherCanvas;
}

function getPadOutStitcherMask(canvas: HTMLCanvasElement | null | undefined): HTMLCanvasElement | null {
  return ((canvas as PadOutStitcherCanvas | null | undefined)?.__imageopsPadOutMask) ?? null;
}
...
function padOutRatio(targetFormat: string): [number, number] | null {
...
function normalizePadOutFillMode(value: string): string {
...
function drawPadOutExtendedEdges(
  ctx: CanvasRenderingContext2D,
  source: HTMLCanvasElement,
  sourceWidth: number,
  sourceHeight: number,
...
function renderPadOutStitchCanvases(
  node: ComfyNode,
  inputs: HTMLCanvasElement[] = [],
  frameIndex: number = 0,
): { image: HTMLCanvasElement; mask: HTMLCanvasElement } {
...
export function renderCompPreview(
  ...
  const outputWidth = useAutoLayering
    ? largestWidth
    : useFirst && firstInput
      ? Math.max(1, firstInput.width)
      : Math.max(1, Math.round(num(node, "width", firstInput?.width ?? 1024)));
  const outputHeight = useAutoLayering
    ? largestHeight
    : useFirst && firstInput
      ? Math.max(1, firstInput.height)
      : Math.max(1, Math.round(num(node, "height", firstInput?.height ?? 1024)));
...
export const ops = {
  ...
  padOutStitcher(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode, inputs: HTMLCanvasElement[] = [], frameIndex: number = 0): HTMLCanvasElement {
    const source = inputs[0] ?? ctx.canvas;
    const { image, mask } = renderPadOutCanvases(node, source, frameIndex, false);
    return markPadOutStitcherCanvas(image, mask);
  },
  padOutStitch(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode, inputs: HTMLCanvasElement[] = [], frameIndex: number = 0): HTMLCanvasElement {
