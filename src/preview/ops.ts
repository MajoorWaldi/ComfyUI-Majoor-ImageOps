...
function numAny(node: ComfyNode, names: string[], fallback: number = 0): number {
  const v = wAny(node, names)?.value;
  const n = parseFloat(v as string);
  return Number.isFinite(n) ? n : fallback;
}
function strAny(node: ComfyNode, names: string[], fallback: string = ""): string {
  const v = wAny(node, names)?.value;
  return typeof v === "string" ? v : fallback;
}
function boolAny(node: ComfyNode, names: string[], fallback: boolean = false): boolean {
  const v = wAny(node, names)?.value;
  if (typeof v === "boolean") return v;
  if (typeof v === "number") return !!v;
  if (typeof v === "string") return v.toLowerCase() === "true";
...
export const ops = {
  ...
  imageOpsMask(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode, cls: string, inputs: HTMLCanvasElement[] = []): HTMLCanvasElement | null {
    const source = inputs[0] ?? ctx.canvas;
    const rawMask = inputs[1] ?? null;
