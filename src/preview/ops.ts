...
export const ops = {
  colorAjust(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode): void {
    ...
    applyColorCorrectReference(ctx, width, height,
  ...
  blur(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode): void {
    const { width, height } = getCanvasDimensions(ctx);
    applyBlur(ctx, width, height, num(node,"radius",0), num(node,"sigma", num(node, "radius", 0)));
  },
  channel(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode): void {
    const { width, height } = getCanvasDimensions(ctx);
    applyChannel(ctx, width, height, str(node, "channel", "Red"));
  },
  crop(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode): HTMLCanvasElement {
    const { width, height } = getCanvasDimensions(ctx);
    return applyCrop(
      ctx,
      node,
  ...
  transform(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode): HTMLCanvasElement | void {
    const { width, height } = getCanvasDimensions(ctx);
    return applyTransform(ctx,width,height,
  ...
  invert(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode, _opts?: any): void {
    const { width, height } = getCanvasDimensions(ctx);
    applyInvert(ctx,width,height, bool(node,"invert_alpha",false));
  },
  clamp(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode): void {
    const { width, height } = getCanvasDimensions(ctx);
  ...
  cropReformat(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode): void {
    ...
    applyCropReformat(ctx,width,height,
      ...
      num(node,"crop_w",width), num(node,"crop_h",height),
      num(node,"padding",0),
  ...
  merge(ctx: CanvasRenderingContext2D, W: number, node: ComfyNode, topCanvas: HTMLCanvasElement, _opts?: any): void {
    const { width, height } = getCanvasDimensions(ctx);
