...
export type CompControlsUi = {
  ...
  layerLabel: HTMLDivElement;
};

export function createCompControlsUi(): CompControlsUi {
...
function removeCompInputAt(node: ComfyNode, inputIndex: number): void {
...
function isCompSlotLinked(node: ComfyNode, slot: ReturnType<typeof getCompSlots>[number]): boolean {
...
export function removeSelectedCompLayer(node: ComfyNode): boolean {
  ...
  const indexes = [selected.inputIndex, selected.maskInputIndex ?? -1]
...
export function writeCompLayers(node: ComfyNode, layers: ReturnType<typeof readCompLayers>, notify: boolean = true): void {
...
export function ensureCompState(node: ComfyNode, notify: boolean = true): ReturnType<typeof readCompLayers> {
...
export function updateCompControls(node: ComfyNode): void {
  ...
  if (st.compAspectRatioSelect) {
    ...
    st.compAspectRatioSelect.title = customFormat
      ...
      : "Aspect ratio is only used when Custom Format is active";
  }
...
export function updateSelectedCompLayer(node: ComfyNode, updater: (layer: ReturnType<typeof readCompLayers>[number]) => void, notify: boolean = true): void {
  const st = ensureState(node);
