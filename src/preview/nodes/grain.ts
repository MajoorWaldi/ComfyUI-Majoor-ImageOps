...
export function createGrainControlsUi(): GrainControlsUi {
  ...
  for (const [value, label] of [["add", "Add"], ["overlay", "Overlay"], ["soft_light", "Soft Light"]] as const) {
  ...
  // Film stock presets — set amount/blend/mono via the existing inputs.
  ...
  for (const preset of presets) {
    ...
    btn.addEventListener("click", (e) => {
      ...
      try { amountInput.dispatchEvent(new Event("input", { bubbles: true })); } catch {}
      try { amountInput.dispatchEvent(new Event("change", { bubbles: true })); } catch {}
      blendSelect.value = preset.blend;
      try { blendSelect.dispatchEvent(new Event("change", { bubbles: true })); } catch {}
      const monoOn = monoButton.textContent?.trim().toLowerCase() === "mono on";
      if (monoOn !== preset.mono) {
        try { monoButton.click(); } catch {}
      }
    });
...
export function syncGrainWidgets(node: ComfyNode): void {
  ...
  const amountLabel = root.querySelector<HTMLElement>('[data-grain-amount-label]');
  ...
  const monoButton = root.querySelector<HTMLButtonElement>('button[data-grain-toggle="monochrome"]');
  if (monoButton) {
    monoButton.textContent = monochrome ? "Mono on" : "Mono off";
    styleSoftButton(monoButton, monochrome);
  }

  const animatedButton = root.querySelector<HTMLButtonElement>('button[data-grain-toggle="animated"]');
  if (animatedButton) {
    animatedButton.textContent = animated ? "Animated on" : "Animated off";
    styleSoftButton(animatedButton, animated);
  }

  const invertButton = root.querySelector<HTMLButtonElement>('button[data-grain-toggle="invert_mask"]');
  if (invertButton) {
    invertButton.textContent = invertMask ? "Invert mask on" : "Invert mask off";
    styleSoftButton(invertButton, invertMask);
  }

  const bypassButton = root.querySelector<HTMLButtonElement>('button[data-grain-toggle="bypass"]');
  if (bypassButton) {
