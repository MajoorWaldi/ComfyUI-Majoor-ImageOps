...
type JoinTrimRowState = {
  ...
  syncTimeline(): void;
  commit(refreshMode?: JoinRefreshMode): void;
};

function inputIndexForSlot(node: ComfyNode, slot: string): number {
...
function createHandle(color: string): HTMLDivElement {
...
function buildRuler(max: number): HTMLDivElement {
...
function selectionText(start: number, end: number, frameCount: number): string {
  return `${start}\u2013${end} (${selectionCount(start, end, frameCount)}f)`;
}

function buildJoinTrimRow(node: ComfyNode, ctx: NodeInteractionContext, st: any, slot: string, index: number): JoinTrimRowState {
  ...
  const rowState: JoinTrimRowState = {
    slot,
    index,
    wrap,
    title,
    ...
    commit(refreshMode: JoinRefreshMode = "now") {
  ...
  rowState.sliderBox.addEventListener("pointerdown", (event: PointerEvent) => {
  ...
  rowState.sliderBox.addEventListener("pointermove", (event: PointerEvent) => {
...
function renderJoinTrimControls(node: ComfyNode, ctx: NodeInteractionContext): void {
  const st = (node as any).__imageops_state as any;
  const list = st?.joinTrimList as HTMLDivElement | null;
  if (!list) return;
  ...
  let rows = st.joinTrimRows as Map<string, JoinTrimRowState> | undefined;
  if (!(rows instanceof Map)) {
    rows = new Map<string, JoinTrimRowState>();
...
export function attachInteractions(node: ComfyNode, ctx: NodeInteractionContext): void {
  ...
  st.joinAddButton?.addEventListener("click", (event: MouseEvent) => {
    event.preventDefault();
...
export function syncJoinControls(node: ComfyNode, ctx: NodeInteractionContext): void {
  renderJoinTrimControls(node, ctx);
}
