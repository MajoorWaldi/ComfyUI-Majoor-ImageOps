...
function isPreviewNode(node: ComfyNode): boolean {
  return String(node?.comfyClass ?? "") === "ImageOpsPreview";
}

function ensureState(node: ComfyNode): NodeState {
...
function styleSoftButton(button: HTMLButtonElement, active: boolean = false): void {
...
function isCropNode(node: ComfyNode): boolean {
...
function getNativePreviewImage(node: ComfyNode): HTMLImageElement | null {
...
function hideNativeMediaPreview(st: NodeState): void {
  if (st.mediaVideo) {
    st.mediaVideo.pause();
...
function parsePreviewSourceUrl(src: string): URL | null {
  try {
...
function nativePreviewFilename(node: ComfyNode): string | null {
...
function buildMediaPreviewUrl(node: ComfyNode, canvasSize: number): string | null {
  const img = getNativePreviewImage(node);
  const parsed = parsePreviewSourceUrl(img?.src ?? "");
...
function showNativeMediaPreview(node: ComfyNode, st: NodeState, canvasSize: number): boolean {
...
function findWidget(node: ComfyNode, name: string) {
  return node?.widgets?.find((w) => w?.name === name) ?? null;
}

function widgetNumber(node: ComfyNode, name: string, fallback: number = 0): number {
...
function widgetBoolean(node: ComfyNode, name: string, fallback: boolean = false): boolean {
  const value = findWidget(node, name)?.value;
...
function resolveCropAspectRatioValue(node: ComfyNode, fallbackWidth: number = 1, fallbackHeight: number = 1): number {
...
function hideWidgetForGood(node: ComfyNode, widget: ComfyWidget | null, suffix: string = ""): void {
  if (!widget) return;
...
function hideCropGeometryWidgets(node: ComfyNode): void {
...
function hideCompWidgets(node: ComfyNode): void {
...
function setWidgetValue(widget: ComfyWidget | null, value: number): void {
  if (!widget) return;
  widget.value = value;
}

function setWidgetStringValue(widget: ComfyWidget | null, value: string): void {
  if (!widget) return;
  widget.value = value;
}

function setWidgetBooleanValue(widget: ComfyWidget | null, value: boolean): void {
  if (!widget) return;
  widget.value = value;
}

function ensureCompInputs(node: ComfyNode, minLayers: number = 1): void {
...
function readCompLayers(node: ComfyNode) {
  return syncCompLayers(findWidget(node, "layers_json")?.value ?? "", getCompSlots(node));
}

function writeCompLayers(node: ComfyNode, layers: ReturnType<typeof readCompLayers>): void {
  setWidgetStringValue(findWidget(node, "layers_json"), serializeCompLayers(layers));
}

function ensureCompState(node: ComfyNode): ReturnType<typeof readCompLayers> {
...
function getCropControlState(node: ComfyNode, fallbackWidth: number = 1, fallbackHeight: number = 1): {
  aspectRatio: number;
  centerX: number;
  centerY: number;
  scale: number;
...
function setCropControlState(node: ComfyNode, centerX: number, centerY: number, scale: number): void {
...
function resetCropControlState(node: ComfyNode): void {
...
function syncDrawWidgets(node: ComfyNode, changedName?: string): void {
...
function updateDrawOverlayWidget(node: ComfyNode): void {
  const st = ensureState(node);
...
function getFitPlacement(width: number, height: number, sourceWidth: number, sourceHeight: number): FitPlacement {
...
function getCompInfoText(node: ComfyNode, connectedLayers: number, totalLayers: number, width: number, height: number): string {
...
function getCropInteractionMode(geometry: CropPreviewGeometry | null, x: number, y: number): CropDragMode | "move" | null {
...
function canvasToDrawSourcePoint(geometry: DrawPreviewGeometry | null, x: number, y: number): { x: number; y: number; inside: boolean } {
...
function updateCompControls(node: ComfyNode): void {
  const st = ensureState(node);
...
function updateSelectedCompLayer(node: ComfyNode, updater: (layer: ReturnType<typeof readCompLayers>[number]) => void): void {
  const st = ensureState(node);
  const layers = ensureCompState(node);
...
function compCanvasToOutputPoint(node: ComfyNode, canvasWidth: number, canvasHeight: number, x: number, y: number): { x: number; y: number } {
  const st = ensureState(node);
...
function getCompHit(node: ComfyNode, canvasWidth: number, canvasHeight: number, x: number, y: number): { layer: CompLayerPreviewGeometry; mode: CompDragMode | "move" } | null {
  const st = ensureState(node);
  const fit = getFitPlacement(canvasWidth, canvasHeight, st.compOutputWidth || 1, st.compOutputHeight || 1);
  const threshold = 10;
...
export function registerImageOpsLivePreview(): void {
  ...
  function ensureDrawCanvasSize(node: ComfyNode, width: number, height: number, persist: boolean = false): void {
    const st = ensureState(node);
  ...
  function drawPointerDynamics(node: ComfyNode, event?: PointerEvent): { size: number; opacity: number } {
  ...
  function paintDrawSegment(node: ComfyNode, fromX: number, fromY: number, toX: number, toY: number, dynamics = { size: 1, opacity: 1 }): void {
    const st = ensureState(node);
  ...
  async function renderDrawMaskCanvas(node: ComfyNode, tick: number): Promise<HTMLCanvasElement> {
    const upstream = getUpstreamNode(node, 0);
    let width = clampDrawDimension(widgetNumber(node, "width", 1024));
    let height = clampDrawDimension(widgetNumber(node, "height", 1024));
    if (upstream) {
  ...
  function attachCompInteractions(node: ComfyNode): void {
    ...
    canvas.addEventListener("pointermove", (event: PointerEvent) => {
      ...
      updateSelectedCompLayer(node, (layer) => {
        ...
        if (drag.mode === "move") {
          ...
          layer.centerX = clampCompCenter(drag.startCenterX + deltaX / Math.max(1, st.compOutputWidth));
          layer.centerY = clampCompCenter(drag.startCenterY + deltaY / Math.max(1, st.compOutputHeight));
          return;
        }
