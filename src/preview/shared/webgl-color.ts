// WebGL-accelerated colour correction — GPU equivalent of
// `applyColorCorrectReference` in ops.ts. Same parameter signature and the
// same maths, but executed in a single fragment shader pass instead of a
// per-pixel JS loop. Falls back gracefully if WebGL is unavailable.
//
// Robustness contract:
//   - All entry points are wrapped in try/catch by the caller; this module
//     never throws across the boundary. Returns false on any failure so the
//     caller can fall back to the CPU implementation.
...
function ensureState(): GLState | null {
  ...
  try {
    ...
    if (!vs || !fs) { _initFailed = true; return null; }

    const program = gl.createProgram();
    if (!program) { _initFailed = true; return null; }
    ...
    if (!buf) { _initFailed = true; return null; }
    ...
    if (!tex) { _initFailed = true; return null; }
...
function wheelTint(hueDeg: number, amount: number): [number, number, number] {
  // The shader treats vec3(0.5) as "no shift" (it does `(tint - 0.5) * mask`).
  // When amount==0 the returned colour MUST therefore be (0.5, 0.5, 0.5) or
  // the preview gets darkened to ~57% by default (the bug behind the
  // "terne" preview report). We bias HSV(c,x,0) around 0.5 so amount=0 is
  // perfectly neutral and amount=100 still reaches the saturated primary.
...
export interface ColorCorrectParams {
  ...
  // Per-zone primaries. Zero / 1.0-anchored for gamma; same units as the
  // global counterparts (-100..100 sliders, 0.2..2.2 for gamma).
  shadowsTemperature?: number;
  shadowsTint?: number;
  shadowsContrast?: number;
  shadowsSaturation?: number;
  shadowsVibrance?: number;
  shadowsGamma?: number;
  shadowsBrightness?: number;
  midtonesTemperature?: number;
  midtonesTint?: number;
  midtonesContrast?: number;
  midtonesSaturation?: number;
  midtonesVibrance?: number;
  midtonesGamma?: number;
  midtonesBrightness?: number;
  highlightsTemperature?: number;
  highlightsTint?: number;
  highlightsContrast?: number;
  highlightsSaturation?: number;
  highlightsVibrance?: number;
  highlightsGamma?: number;
  highlightsBrightness?: number;
  lumaWeights: [number, number, number];
}

export function isWebGLColorAvailable(): boolean {
  return ensureState() != null;
}

/**
 * Apply colour correction to `targetCtx` (in-place). Returns true on success,
 * false if the GPU path failed for any reason (caller should fall back to CPU).
 */
export function applyColorCorrectGL(
  targetCtx: CanvasRenderingContext2D,
  width: number,
  height: number,
  params: ColorCorrectParams,
): boolean {
  const st = ensureState();
  if (!st) return false;
  const { gl, program, buf, texture, uniforms } = st;
  if (gl.isContextLost()) { _state = null; return false; }

  // Pre-compute mean luma (matches the reference path) — cheap downsampled
  // pass on the source ImageData, then upload the texture.
  let src: ImageData;
  try {
  ...
  // Fast luma estimate over a stride to avoid scanning every pixel.
  ...
  if (sampled === 0) {
    // Fallback to full-resolution scan only if the strided sample didn't fire.
  ...
  try {
    ...
    // Upload the source image directly from the ImageData we already grabbed.
    ...
    // Per-zone primaries packed as vec4 (units match the global path: /100
    // for the linear sliders and gamma-1 for the multiplicative one).
    const z = (v: number | undefined) => (typeof v === "number" ? v / 100 : 0);
    const gz = (v: number | undefined) => (typeof v === "number" ? v - 1 : 0);
  ...
  // Blit GL canvas back into targetCtx. Use a pooled tmp canvas to avoid
  // an extra allocation per frame. drawImage of a WebGL canvas copies its
  // current contents synchronously; no need for readPixels.
  try {
  ...
  // Touch releaseCanvas so the import isn't tree-shaken; also keeps the
  // module in sync with future enhancements that may pool the GL canvas.
  void releaseCanvas;
  return true;
}
