...
import type { ScopesOptions, ScopesResult } from "../types.js";

function clamp01(x: number): number { return Math.max(0, Math.min(1, x)); }

function luma(r: number, g: number, b: number, lw: number[]): number {
  return lw[0] * r + lw[1] * g + lw[2] * b;
}

export function computeScopes(imageData: ImageData, opts?: ScopesOptions): ScopesResult {
...
export function drawHistogram(ctx: CanvasRenderingContext2D, W: number, H: number, hist: Uint32Array): void {
...
export function drawRgbWaveform(ctx: CanvasRenderingContext2D, W: number, H: number, wr: Uint32Array, wg: Uint32Array, wb: Uint32Array, wfW: number, wfH: number): void {
  ...
  for (let y = 0; y < H; y++) {
    ...
    for (let x = 0; x < W; x++) {
      const sx = Math.min(wfW - 1, Math.floor((x / Math.max(1, W - 1)) * (wfW - 1)));
      const sIdx = sy * wfW + sx;
      const ar = Math.max(0, Math.min(255, ((wr[sIdx] / maxV) * 255) | 0));
      const ag = Math.max(0, Math.min(255, ((wg[sIdx] / maxV) * 255) | 0));
      const ab = Math.max(0, Math.min(255, ((wb[sIdx] / maxV) * 255) | 0));
      const idx = (y * W + x) * 4;

      // tint by dominant channel
      if (ar >= ag && ar >= ab) { d[idx] = 255; d[idx + 1] = 80; d[idx + 2] = 80; }
      else if (ag >= ar && ag >= ab) { d[idx] = 80; d[idx + 1] = 255; d[idx + 2] = 80; }
      else { d[idx] = 80; d[idx + 1] = 160; d[idx + 2] = 255; }
      d[idx + 3] = Math.max(ar, ag, ab);
    }
  }
  ctx.putImageData(img, 0, 0);
}
