...
function ensurePreviewWidget(node, progress, canvasSize) {
  ...
  if (colorCorrectNode) {
    ...
    colorHueLabel.textContent = "Hue 0\xB0";
...
function setCornerPinHandle(node, handle, xNorm, yNorm) {
  if (handle === "tl") {
...
function registerImageOpsLivePreview() {
  ...
  function refreshPadOutInteraction(node) {
    const st = ensureState(node);
    st.nativeDirty = true;
    markPreviewInteraction(node);
    markCanvasDirty();
  ...
  function refreshCornerPinInteraction(node) {
    const st = ensureState(node);
    st.nativeDirty = true;
    markPreviewInteraction(node);
    markCanvasDirty();
  ...
  function attachPreviewInteractions(node) {
    if (!isPreviewNode(node)) return;
    const st = ensurePreviewWidget(node, progress, canvasSize);
    const root = st?.canvas?.parentElement;
  ...
  function attachColorCorrectInteractions(node) {
    if (!isColorCorrectNode(node)) return;
    const st = ensurePreviewWidget(node, progress, canvasSize);
  ...
  function attachCompInteractions(node) {
    ...
    canvas.addEventListener("pointermove", (event) => {
      ...
      updateSelectedCompLayer(node, (layer) => {
        ...
        const anchorOffset = drag.mode === "nw" ? { x: halfWidth, y: halfHeight } : drag.mode === "ne" ? { x: -halfWidth, y: halfHeight } : drag.mode === "sw" ? { x: halfWidth, y: -halfHeight } : { x: -halfWidth, y: -halfHeight };
        ...
        const corner = drag.mode === "nw" ? { x: localAnchor.x - width, y: localAnchor.y - height } : drag.mode === "ne" ? { x: localAnchor.x + width, y: localAnchor.y - height } : drag.mode === "sw" ? { x: localAnchor.x - width, y: localAnchor.y + height } : { x: localAnchor.x + width, y: localAnchor.y + height };
        const centerLocal = {
  ...
  function attachCornerPinInteractions(node) {
  ...
  function attachPadOutInteractions(node) {
