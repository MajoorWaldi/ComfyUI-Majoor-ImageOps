  ...
  return [
    {
      ...
      match(node) {
        ...
        return (node.widgets ?? []).some((w) => w?.name === "in_min") && (node.widgets ?? []).some((w) => w?.name === "in_max") && (node.widgets ?? []).some((w) => w?.name === "gamma");
      },
      inputs: 1,
      async apply({ ctx, canvasSize, node }) {
        ops.levels(ctx, canvasSize, node, { wasCompat: true });
      }
    },
    {
      name: "was:huesat",
      ...
      inputs: 1,
      async apply({ ctx, canvasSize, node }) {
        ops.hueSat(ctx, canvasSize, node, { wasCompat: true });
      }
    },
    {
      name: "was:invert",
    ...
    },
    {
      ...
      async apply({ ctx, canvasSize, node, inputs }) {
        ops.merge(ctx, canvasSize, node, inputs[1], { wasCompat: true });
      }
    }
  ];
}
