...
function getColorWheelSwatchCss(hueDeg, saturationPercent) {
...
function colorWheelPointToValues(canvasX, canvasY, canvas) {
  const width = Math.max(1, canvas.width || 1);
  const height = Math.max(1, canvas.height || 1);
  const centerX = width / 2;
  const centerY = height / 2;
...
export {
  colorWheelPointToValues,
  drawColorWheel,
  getColorWheelSwatchCss
};
