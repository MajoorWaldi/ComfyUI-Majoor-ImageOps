...
import { hideWidgetsByName, setWidgetStringValuesByName, widgetBoolean, widgetNumber, widgetString } from "../shared/widgets.js";
...
function createTextControlsUi() {
  ...
  const makeEyedropperButton = (getInput) => {
  ...
  posWrap.appendChild(makeNumberInput("x", { min: -2, max: 3, step: 1e-3, title: "Normalized X" }));
  posWrap.appendChild(makeNumberInput("y", { min: -2, max: 3, step: 1e-3, title: "Normalized Y" }));
...
function syncTextWidgets(node) {
  ...
  const setNumberField = (name, value) => {
    const input = root.querySelector(`input[data-text-field="${name}"]`);
  ...
  const fontPathInput = root.querySelector('input[data-text-field="font_path"]');
  if (fontPathInput && fontPathInput.value !== fontPath) fontPathInput.value = fontPath;
  const opacityInput = root.querySelector('input[data-text-field="opacity"]');
  if (opacityInput) opacityInput.value = String(Math.round(opacity * 100));
  const opacityLabel = root.querySelector("[data-text-opacity-label]");
  ...
  const colorInput = root.querySelector('input[data-text-color="fill"]');
  if (colorInput) {
    colorInput.value = color;
    syncDarkColorInputUI(colorInput, color);
  }
  const strokeColorInput = root.querySelector('input[data-text-color="stroke"]');
  if (strokeColorInput) {
    strokeColorInput.value = strokeColor;
    syncDarkColorInputUI(strokeColorInput, strokeColor);
  }
  const invertButton = root.querySelector('button[data-text-toggle="invert_mask"]');
  if (invertButton) {
    invertButton.textContent = invertMask ? "Invert mask on" : "Invert mask off";
    styleSoftButton(invertButton, invertMask);
  }
  const bypassButton = root.querySelector('button[data-text-toggle="bypass"]');
  if (bypassButton) {
