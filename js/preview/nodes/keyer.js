...
function ensureKeyColorsWidget(node) {
  let widget = findWidget(node, "key_colors");
  if (widget) return widget;
  const created = node.addWidget?.("text", "key_colors", "", () => {
  }, { serialize: true });
  if (created) {
...
function parseKeyColorSelection(node) {
...
function writeKeyColorSelection(node, colors) {
...
function setRange(input, value) {
  ...
  const label = input.parentElement?.querySelector("[data-keyer-value]");
...
function syncKeyerWidgets(node) {
  ...
  setRange(st.keyerGainInput, widgetNumber(node, "gain", 1));
  setRange(st.keyerBlurInput, widgetNumber(node, "blur", 0));
...
function sampleCanvasColor(canvas, event) {
...
function applyPickedColor(node, sampledColor, event) {
...
function attachKeyerControls(node, ctx) {
  if (!isNode(node)) return;
  ensureKeyColorsWidget(node);
  const st = node.__imageops_state;
  ...
  const bindRange = (input, widgetName) => {
  ...
  st.canvas?.addEventListener("pointerdown", (event) => {
