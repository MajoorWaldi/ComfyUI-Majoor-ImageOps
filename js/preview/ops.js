...
const ops = {
  ...
  blur(ctx, W, node) {
    applyBlur(ctx, W, W, num(node, "radius", 0));
  },
  ...
  invert(ctx, W, node, _opts) {
    applyInvert(ctx, W, W, bool(node, "invert_alpha", false));
  },
  clamp(ctx, W, node) {
    applyClamp(ctx, W, W, num(node, "min_v", 0), num(node, "max_v", 1));
  },
  sharpen(ctx, W, node) {
    applyUnsharp(ctx, W, W, num(node, "amount", 1));
  },
  edgeDetect(ctx, W, node) {
    applyEdgeDetect(ctx, W, W, num(node, "strength", 1));
  },
  glow(ctx, W, node) {
    applyGlow(ctx, W, W, num(node, "threshold", 0.8), num(node, "intensity", 0.75), Math.round(num(node, "blur_px", 6)));
  },
  ...
  lumaKey(ctx, W, node) {
    applyLumaKey(ctx, W, W, num(node, "low", 0.1), num(node, "high", 0.9), num(node, "softness", 0.05));
  },
  merge(ctx, W, node, topCanvas, _opts) {
    blend(ctx, W, W, topCanvas, str(node, "mode", "over"), num(node, "mix", 1));
  }
};
export {
  ops
