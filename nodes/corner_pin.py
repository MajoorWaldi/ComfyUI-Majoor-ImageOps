...
def _build_corner_pin_grid(height: int, width: int, Hinv: torch.Tensor, supersample: int=1) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    denom = torch.where(torch.abs(denom) < 1e-08, safe_sign * 1e-08, denom)
...
def _corner_pin_dst_points(batch: int, height: int, width: int, device, dtype, tl_x, tl_y, tr_x, tr_y, bl_x, bl_y, br_x, br_y) -> torch.Tensor:
    ...
    points = torch.stack([torch.stack([_param_tensor(tl_x, batch, device, dtype).view(batch) * w, _param_tensor(tl_y, batch, device, dtype).view(batch) * h], dim=1), torch.stack([_param_tensor(tr_x, batch, device, dtype).view(batch) * w, _param_tensor(tr_y, batch, device, dtype).view(batch) * h], dim=1), torch.stack([_param_tensor(bl_x, batch, device, dtype).view(batch) * w, _param_tensor(bl_y, batch, device, dtype).view(batch) * h], dim=1), torch.stack([_param_tensor(br_x, batch, device, dtype).view(batch) * w, _param_tensor(br_y, batch, device, dtype).view(batch) * h], dim=1)], dim=1)
    return points
...
def _warp_corner_pin_group(input_nchw: torch.Tensor, Hinv: torch.Tensor, height: int, width: int, mode: str, padding: str, supersample: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    warped = F.grid_sample(input_nchw, grid, mode=mode, padding_mode=padding, align_corners=True)
    if supersample > 1:
...
class ImageOpsCornerPin(io.ComfyNode):
    ...
    def execute(cls, image=None, bypass=False, tl_x=0.0, tl_y=0.0, tr_x=1.0, tr_y=0.0, bl_x=0.0, bl_y=1.0, br_x=1.0, br_y=1.0, filter='bilinear', supersample=1, fill_mode='transparent', fill_color='#000000', edge_mode='transparent', invert_mask=False, video=None, unique_id=None, **kwargs):
        ...
        for frame_index in range(batch):
            key = (_normalize_filter(_scalar(filter, str, index=frame_index)), _normalize_padding(_scalar(fill_mode, str, index=frame_index), _scalar(edge_mode, str, index=frame_index)), _normalize_supersample(_scalar(supersample, int, index=frame_index)))
        ...
        for (mode, padding, sample_factor), indices in groups.items():
            ...
            group_warped, group_valid = _warp_corner_pin_group(input_nchw.index_select(0, index_tensor), Hinv.index_select(0, index_tensor), height, width, mode, padding, sample_factor)
