...
def _flip_tensor_spatial(tensor: torch.Tensor, flip_mode):
    if isinstance(flip_mode, (list, tuple)):
        if tensor.shape[0] == 0:
            return tensor
        return torch.cat([_flip_tensor_spatial(tensor[index:index + 1], _normalize_flip_mode(flip_mode, index=index)) for index in range(tensor.shape[0])], dim=0)
...
def _transform_batch_affine(source: torch.Tensor, filter_mode, translate_x, translate_y, rotate_deg, scale, padding_mode: str='zeros') -> torch.Tensor:
    """Batched GPU affine transform for a [B, H, W, C] image tensor."""
    B, H, W, C = source.shape
    if isinstance(filter_mode, (list, tuple)):
        if B == 0:
            return source
        return torch.cat([_transform_batch_affine(source[i:i + 1], _scalar(filter_mode, str, index=i), _frame_param(translate_x, i), _frame_param(translate_y, i), _frame_param(rotate_deg, i), _frame_param(scale, i), padding_mode=padding_mode) for i in range(B)], dim=0)
...
def _transform_mask_affine(mask: torch.Tensor, filter_mode, translate_x, translate_y, rotate_deg, scale, device: torch.device, dtype: torch.dtype, padding_mode: str='zeros') -> torch.Tensor:
    ...
    if isinstance(filter_mode, (list, tuple)):
        ...
        return torch.cat([_transform_mask_affine(mask[i:i + 1], _scalar(filter_mode, str, index=i), _frame_param(translate_x, i), _frame_param(translate_y, i), _frame_param(rotate_deg, i), _frame_param(scale, i), device, dtype, padding_mode=padding_mode) for i in range(B)], dim=0)
    mode = _filter_to_grid_sample_mode(filter_mode)
                ...
                for idx in range(len(value)):
                    ...
                    if kind == 'bool':
                        ...
                        if abs(int(round(float(current)))) > 0:
                            return False
                    elif abs(float(current)) > EPSILON:
                        return False
                return True
