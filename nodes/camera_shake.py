...
def _shake_params(frames: int, translate_px: float, rotate_deg: float, zoom: float, seed: int, smoothing: float, shake_frequency: float) -> tuple[list[float], list[float], list[float], list[float]]:
...
class ImageOpsCameraShake(io.ComfyNode):
    ...
    def execute(cls, image=None, bypass=False, translate_px=12.0, rotate_deg=1.5, zoom=0.03, smoothing=0.65, shake_frequency=1.0, frame_length=24, fps=12.0, seed=12345, filter='bilinear', fill_mode='mirror', fill_color='#000000', invert_mask=False, video=None, mask=None, unique_id=None, **kwargs):
        ...
        tx, ty, rot, scales = _shake_params(frames, _scalar(translate_px), _scalar(rotate_deg), _scalar(zoom), _scalar(seed, int), _scalar(smoothing), _scalar(shake_frequency))
        safe_fill = _normalize_fill_mode(fill_mode)
        padding = _padding_mode_from_fill(safe_fill)
        if input_mask is not None:
            result, output_mask = _transform_masked_source(source, input_mask, filter, tx, ty, rot, scales, progress=progress, padding_mode=padding)
            ...
            output_mask = _transform_mask_affine(output_mask_source, filter, tx, ty, rot, scales, source.device, source.dtype, padding_mode='zeros')
            result = _composite_fill(result, output_mask, _make_fill_background(source, safe_fill, fill_color))
